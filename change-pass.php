...
session_start();
include('db_conn.php');
if((!isset($_POST['submit_forgot_pass_otp'])) && (!isset($_POST['submit_change_pass'])))
{
 header("location:forgot-pass.php");
}
if (isset($_POST["submit_forgot_pass_otp"]))
{
		if( $_SESSION['both_otp'] != $_POST["otp"])
		{
        ...
        if($password != $c_password)
		{
			echo '<script type="text/javascript">
					window.onload = function () 
					{ alert("Password and'.$_SESSION['check_id'].' Confirm password not matched !"); }
					</script>';
		}
		else{
			$sql = "UPDATE `seller_login` SET `password` = '".$password."'  WHERE id = ".$_SESSION['check_id'];	
					...
					<form class="s12" method="POST" action="">
						...
						<div>
							<div class="input-field s4">
								<input type="submit" name="submit_change_pass" value="Change Password" class="waves-effect waves-light log-in-btn" > </div>
						</div>
